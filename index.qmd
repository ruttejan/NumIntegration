...
-   Pro numerickou integraci můžeme využít několika metod:
:::
...
::: uvod
-   Numerické integrace užijeme v případech, kdy je integrand příliš složitý nebo nelze vypočítat
...
$$x_0 = a; x_i = a + i \cdot h; x_n = b,\space\space kde \space\space i = 1\dots n-1$$

$$\int_{a}^{b} f(x) \, dx \approx \sum_{i=1}^{n} f\left(\frac{x_{i-1} + x_i}{2}\right) h$$
:::

## Výpočet - obdélníkové metody
...
$$x_0 = a;\space x_i = a + i \cdot h;\space x_n = b,\space\space kde \space\space i = 1\dots n-1$$

$$\int_{a}^{b} f(x) \, dx \approx \frac{h}{2} (x_0 + x_n + \sum_{i=1}^{n-1} 2f(x_i))$$

:::

## Výpočet - lichoběžníkové metody
...
-   Aproximace integrálu se zjednodušší na výpočet intrgrálu z kvadratické funkce
$$h = \frac{a - b}{n}$$
$$x_0 = a;\space x_i = a + i \cdot h;\space x_n = b,\space\space kde \space\space i = 1\dots n-1$$
...
-   Jelikož všechny krajní body podintervalů (kromě krajních bodů původního intervalu) použijeme dvakrát, proto jim přiřadíme váhu 2 [pro více informací: @navara2023]
$$\int_{a}^{b} f(x) \, dx \approx \frac{h}{6}(x_0 + x_n + \sum_{i=1}^{n-1} 2f(x_i) + \sum_{i=0}^{n-1} 4f(x_i + \frac{h}{2}))$$ 

:::

## Výpočet - Simpsonovy metody
...
::: txt2
- Pro lepší zobrazení rozdílu metod použijeme pouze funkce **numerical_int()**
...
:::{#refs}
<!-- -   Calculus Early Transcendentals: Integral & Multi-Variable Calculus for Social Sciences: 2.6 Numerical Integration [online]. [cit. 2023-11-19]. Dostupné z: <https://www.sfu.ca/math-coursenotes/Math%20158%20Course%20Notes/sec_Numerical_Integration.html>

-   NAVARA, Mirko. Numerické metody [online]. 2023, 31.10.2023, 60-64 [cit. 2023-11-19]. Dostupné z: <https://cmp.felk.cvut.cz/~navara/nm/NMall_print.pdf> -->
:::